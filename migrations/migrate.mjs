...
async function main() {
    ...
    if (!(source in databases) || !(destination in databases)) {
        ...
        console.log('Options for source/destination: production, testDB, localhost');
        process.exit(1);
    }
    ...
    const collectionsSource = await getCollections(sourceConfig.uri, sourceConfig.name);
    const collectionsDest = await getCollections(destConfig.uri, destConfig.name);

    const onlyInSource = [...collectionsSource].filter(x => !collectionsDest.has(x));

    console.log(`Collections only in ${source}:`, onlyInSource);

    if (onlyInSource.length > 0) {
        console.log(`Creating collections in ${destination} that are only in ${source}...`);
