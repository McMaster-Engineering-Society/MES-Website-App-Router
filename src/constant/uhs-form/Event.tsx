        ...
        <Question title="Start Date" required={true}>
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DatePicker
              ...
              onChange={(date) => updateFields({ startDate: dayjs(date).format('YYYY-MM-DD') })}
              slotProps={{
        ...
        <Question title="Start Time" required={true}>
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <TimePicker
              ...
              onChange={(time) => updateFields({ startTime: dayjs(time).format('HH:mm') })}
              slotProps={{
                textField: { sx: textFieldProps } as TextFieldProps,
                layout: {
                  sx: {
        ...
        <Question title="End Date" description="If it's a repeating event, please put the end date of the first repeat (Eg for 1h weekly meetings starting on Feb 1 and ending Mar 1, the end date would be Feb 1 not Mar 1)" required={true}>
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DatePicker
              ...
              name="endDate"
              value={dayjs(endDate)}
              onChange={(date) => updateFields({ endDate: dayjs(date).format('YYYY-MM-DD') })}
              slotProps={{
                textField: { sx: textFieldProps } as TextFieldProps,
                layout: {
                  sx: {
        ...
        <Question title="End Time" required={true}>
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <TimePicker
              ...
              format="hh:mm a"
              value={dayjs(endTime)}
              onChange={(time) => updateFields({ endTime: dayjs(time).format('HH:mm') })}
              slotProps={{
                textField: { sx: textFieldProps } as TextFieldProps,
                layout: {
                  sx: {
