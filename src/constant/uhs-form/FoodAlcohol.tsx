...
function FoodAlcohol({ food, alcohol, place, seventhAgreement, updateFields }: FoodAlcoholProps) {
  return (
      ...
      <div id='text-body' className='flex flex-col gap-x-4 gap-y-4'>
        <Question title="Does your event include food & beverages?" description="Please note that if your event is at a venue where food and beverages can be purchased by attendees (eg Boston Pizza), that would be included under an event including food/beverages. Campus Food and Beverage Policy: https://vpof.mcmaster.ca/app/uploads/2021/04/Campus-Food-Beverage-Policy.pdf" required={true}>
          <label className="flex flex-row mt-2 h-fit justify-items-start content-center">
            <input required className="mr-2 w-6 h-6 opacity-50 bg-[#ececec] rounded-[10px] border-2 border-black/40" type="radio" name='food' value="foodYes" checked={food === true} onChange={() => updateFields({ food: true })}/>
          ...
          <label className="flex flex-row mt-2 h-fit justify-items-start content-center">
            <input  className="mr-2 w-6 h-6 opacity-50 bg-[#ececec] rounded-[10px] border-2 border-black/40" type="radio" name='food' value="foodNo" checked={food === false} onChange={() => updateFields({ food: false })}/>
        ...
        <Question title="Is alcohol being served at this event?" required={true}>
          <label className="flex flex-row mt-2 h-fit justify-items-start content-center">
            <input required  className="mr-2 w-6 h-6 opacity-50 bg-[#ececec] rounded-[10px] border-2 border-black/40" type="radio" name='alcohol' value="alcoholYes" checked={alcohol === true} onChange={() => updateFields({ alcohol: true })}/>
            <div className="text-[#4b4b4b] h-fit text-lg font-medium font-['Inter']">Yes</div>
          </label>
          <label className="flex flex-row mt-2 h-fit justify-items-start content-center">
            <input  className="mr-2 w-6 h-6 opacity-50 bg-[#ececec] rounded-[10px] border-2 border-black/40" type="radio" name='alcohol' value="alcoholNo" checked={alcohol === false} onChange={() => updateFields({ alcohol: false})}/>
            <div className="text-[#4b4b4b] h-fit text-lg font-medium font-['Inter']">No</div>
          </label>
        </Question>
        ...
        <Question title="Is the event being held at TwelveEighty, The Pheonix, Paradaise Catering, or Ron Joyce Centre" required={true}>
          <label className="flex flex-row mt-2 h-fit justify-items-start content-center">
            <input required  className="mr-2 w-6 h-6 opacity-50 bg-[#ececec] rounded-[10px] border-2 border-black/40" type="radio" name='place' value="placeYes" checked={place === true} onChange={() => updateFields({ place: true})}/>
            <div className="text-[#4b4b4b] h-fit text-lg font-medium font-['Inter']">Yes</div>
          </label>
          <label className="flex flex-row mt-2 h-fit justify-items-start content-center">
            <input  className="mr-2 w-6 h-6 opacity-50 bg-[#ececec] rounded-[10px] border-2 border-black/40" type="radio" name='place' value="placeNo" checked={place === false} onChange={() => updateFields({ place: false })}/>
            <div className="text-[#4b4b4b] h-fit text-lg font-medium font-['Inter']">No</div>
          </label>
        </Question>
