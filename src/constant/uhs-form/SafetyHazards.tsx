...
function SafetyHazards({ fifthAgreement, involveHazard, updateFields }: SafetyHazardsProps) {
  return (
    <div className="flex flex-col">
      <div id='text-body' className="flex flex-col gap-x-4 gap-y-4 text-[#4b4b4b] text-lg font-medium font-['Inter']">
        ...
        <Question title="Does your event involve a safety hazard as outlined above?" required={true}>
          <label className="flex flex-row mt-2 h-fit justify-items-start content-center">
            <input required className="mr-2 w-6 h-6 opacity-50 bg-[#ececec] rounded-[10px] border-2 border-black/40" type="radio" name='involveHazard' value="involveHazardYes" checked={involveHazard === true} onChange={() => updateFields({ involveHazard: true })}/>
            <div className="text-[#4b4b4b] h-fit text-lg font-medium font-['Inter']">Yes</div>
          </label>
          <label className="flex flex-row mt-2 h-fit justify-items-start content-center">
            <input className="mr-2 w-6 h-6 opacity-50 bg-[#ececec] rounded-[10px] border-2 border-black/40" type="radio" name='involveHazard' value="involveHazardNo" checked={involveHazard === false} onChange={() => updateFields({ involveHazard: false })}/>
